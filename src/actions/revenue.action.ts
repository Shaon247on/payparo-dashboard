...
import type { RevenueStats } from "@/types/revenue.type";
...
export async function getRevenueStatsAction(): Promise<ActionResult<RevenueStats>> {
  const token = await getValidAccessToken();
  if (!token) redirect("/login");
  return adminFetch<RevenueStats>("/api/administration/revenue/", token);
}
